...
//Creating the test objects for hourlyCost var
hourlyCost0 = {hour: 0, cost: 10}; 
...
//Creating the test objects for hourlyUsage var
hourlyUsage0 = {hour: 0, usage: 10};
...
  //Adding the test objects into usage array var
hourlyUsageArray =  []; 
...
//Adding the test objects into the cost array var
hourlyCostArray = [];
...
//Adding relevant hours specified by user var
lowerEnd = 6; 
var upperEnd = 22;
  //Split arrays into relevant hours var
relevantHoursCostArray = [];
...
  //Quick sorting the array in ascending
      order relevantHoursCostArray.sort(function(a, b) { 
...
  //console.log(relevantHoursCostArray);
 //console.log((relevantHoursUsageArray));  
   //Get the top 5 hours from the total 24 hourlyUsageArray and hourlyCostArray var
   top5HourlyCostArray = []; 
...
 var top5HourlyUsageArray = []; 
        for (var i = relevantHoursUsageArray.length - 5; i < relevantHoursUsageArray.length; i++) { 
             ...
             //Get the bottom 5 hours from the total 24 hourlyUsageArray and hourlyCostArray var
             bottomHourlyCostArray = []; 
...
 for (var i = 0; i < top5HourlyUsageArray.length; i++) { 
                 var usageHour = top5HourlyUsageArray[i].hour; 
                  for (var j = 0; j < top5HourlyCostArray.length; j++) { 
                    ...
                    //Checking for intersections between bottom hourly costs and top 5 hourly usage for (var i = 0; i < top5HourlyUsageArray.length; i++) {     var usageHour = top5HourlyUsageArray[i].hour;     for (var j = 0; j < bottomHourlyCostArray.length; j++) {         if (usageHour == bottomHourlyCostArray[j].hour) {             //console.log(bottomHourlyCostArray[j].hour);         }     } }
